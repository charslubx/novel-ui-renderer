...
describe("X renderers",()=>{
  ...
  it("renders a dark single post with media and an inline SVG avatar",()=>{expect(XPostRenderer.validate(post)).toBe(true);const result=XPostRenderer.render({...post,avatar:"https://example.com/avatar.jpg"},{raw:"",debug:false});expect(result.querySelector(".x-media")).not.toBeNull();expect(result.querySelector(".x-avatar svg")).not.toBeNull();expect(result.querySelector(".x-avatar img")).toBeNull();});
