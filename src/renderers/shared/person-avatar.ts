...
export function renderDefaultAvatar(className = "default-avatar", label = "默认头像"): HTMLElement {
  ...
  const outer=document.createElementNS(svg.namespaceURI,"circle"); outer.setAttribute("cx","32"); outer.setAttribute("cy","32"); outer.setAttribute("r","27");
  ...
  svg.append(outer,head,shoulders); avatar.append(svg); return avatar;
}
