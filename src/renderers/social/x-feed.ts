...
export const XFeedRenderer: NovelUIRenderer<XFeedProps> = {
  component: "social",
  variant: "x-feed",
  styles: common + styles,
  validate(value): value is XFeedProps {
    const p = value as XFeedProps;
    return (
      !!p &&
      Array.isArray(p.posts) &&
      p.posts.every(isXPostProps) &&
      (p.title === undefined || typeof p.title === "string")
    );
  },
  render(props) {
    const shell = element("section", "novel-ui x-shell"),
      header = element("header", "x-app-header", props.title ?? "首页");
    header.append(
