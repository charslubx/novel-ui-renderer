...
export const WeiboRenderer: NovelUIRenderer<WeiboProps> = {
  ...
  validate(value): value is WeiboProps {
    const p = value as WeiboProps;
    return (
      !!p &&
      [p.displayName, p.timestamp, p.text].every(
        (x) => typeof x === "string",
      ) &&
      [p.reposts,p.likes].every(optionalNumber) &&
  ...
  render(props) {
    ...
    const meta = [props.timestamp, props.source ? `来自 ${props.source}` : ""]
      .filter(Boolean)
      .join(" · ");
    ...
    [
      ...
      stats.append(element("span", "", `${label} ${value ?? 0}`)),
    );
    root.append(
      header,
