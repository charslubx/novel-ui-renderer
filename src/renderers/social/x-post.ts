...
const isMedia = (value: unknown): value is XMedia => {
  const media = value as XMedia;
  return (
    !!media &&
    (media.type === "image" || media.type === "link") &&
    [media.url, media.alt, media.title, media.description, media.domain].every(
      isText,
    )
  );
};
export function isXPostProps(value: unknown): value is XPostProps {
  const post = value as XPostProps;
  return (
    !!post &&
    [post.displayName, post.handle, post.text, post.timestamp].every(
    ...
    (post.media === undefined ||
      (Array.isArray(post.media) && post.media.every(isMedia)))
  );
}
...
function avatar(props: XPostProps) {
  return renderDefaultAvatar("x-avatar", `${props.displayName}的默认头像`);
}
function renderMedia(items: XMedia[]) {
...
const compact = (value = 0) =>
  value >= 10000
    ...
    : String(value || "");
export function renderXPost(props: XPostProps) {
  const root = element("article", "x-post");
  root.append(avatar(props));
  const body = element("div", "x-post__body"),
    header = element("header", "x-post__author");
  ...
  if (props.media?.length) body.append(renderMedia(props.media));
